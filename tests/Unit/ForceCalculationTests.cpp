...
// --- Helper: vector norm ---
double norm3D(const std::array<double, 3> &v) { return std::sqrt(v[0] * v[0] + v[1] * v[1] + v[2] * v[2]); }
...
TEST(LennardJonesBehaviourTest, RepulsiveForShortDistances) {
  ...
  std::array<double, 3> r12 = {p2.getX()[0] - p1.getX()[0], p2.getX()[1] - p1.getX()[1], p2.getX()[2] - p1.getX()[2]};
