  ...
  if (!OpenMPCompat::enabled()) {
    for (std::size_t base = 0; base < total; ++base) {
      ...
      for (const auto &s : shifts) {
        ...
        if (nx >= static_cast<int>(padded_dims[0]) ||
            ny >= static_cast<int>(padded_dims[1]) ||
            nz >= static_cast<int>(padded_dims[2])) continue;

        const std::size_t nb = toLinearIndex(static_cast<std::size_t>(nx),
