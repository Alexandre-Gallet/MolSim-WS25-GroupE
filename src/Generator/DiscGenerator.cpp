...
void DiscGenerator::generate(Container &container) const {
...
void DiscGenerator::generateDisc(Container &container, const std::array<double, 3> &center, int radiusCells, double h,
                                 double mass, const std::array<double, 3> &baseVelocity, int type) {
  DiscGenerator disc_generator(center, radiusCells, h, mass, baseVelocity, type);
  disc_generator.generate(container);
}
